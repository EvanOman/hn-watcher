...
class SystemdManager:
    ...
    def _detect_uv_project(self, working_dir: Path) -> bool:
        ...
        return (working_dir / "pyproject.toml").exists() and shutil.which("uv") is not None
    ...
    def add(self, name: str, command: str, schedule: str = "*/15", 
            description: str = "", working_dir: Optional[str] = None,
            environment: Optional[Dict[str, str]] = None,
            data_dirs: Optional[List[str]] = None) -> None:
        """
        ...
        try:
            ...
            # Write timer file  
            with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.timer') as f:
                f.write(timer_content)
                temp_timer = f.name
            
            try:
                # Copy files to systemd directory
                subprocess.run(["sudo", "cp", temp_service, str(service_file)], check=True)
    ...
    def update(self, name: str, command: Optional[str] = None, 
              schedule: Optional[str] = None, description: Optional[str] = None,
    ...
    def list_services(self) -> None:
        ...
        for name, config in services.items():
            ...
            # Get status
            try:
                timer_result = self._run_systemctl(["is-active", timer_name], check=False)
                timer_status = timer_result.stdout.strip()
                
                service_result = self._run_systemctl(["is-failed", f"{service_name}.service"], check=False)
                service_failed = service_result.returncode == 0
                
                status_icon = "✗" if service_failed else ("✓" if timer_status == "active" else "○")
                
            except Exception:
                status_icon = "?"
                timer_status = "unknown"
            
            print(f"{status_icon} {name:20} {config['schedule']:15} {timer_status:10} {config['description']}")
    ...
    def logs(self, name: Optional[str] = None, follow: bool = False, 
             lines: int = 50, all_services: bool = False) -> None:
        """Show logs for services."""
        services = self._load_services()
        
        if all_services:
            # Show logs for all services
            identifiers = [f"--identifier={svc['service_name']}" for svc in services.values()]
            if not identifiers:
